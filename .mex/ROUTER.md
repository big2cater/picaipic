...
- **Smart tags 6-bucket + thr (2026-07-24):** people/pets/landscape/architecture/plants/birds; people=`a photo of people`, pets=common species list; default `thresholdIndex=1` (High, new installs); Settings thr re-runs smart-tag CLIP path directly + numeric coerce; not a detector (face index for identities) — `patterns/change-smart-tags.md`, `change-ai-search-filters.md`.
	- **Track C product C (2026-07-24):** bundled **bilingual int8** text tower as `resources/models/text_model.onnx` (EN CLIP text removed from install); **no Settings model switch**; optional cloud re-download for observation; self-host `picaipic-binaries` tag `models`. Vision stays CLIP B/32 (**no reindex**). Guide `docs/guide/altclip-phase0-probe.md`. Pattern `change-image-search-model.md`.
	- **AI search stop-bleed + matrix + ANN (2026-07-24):** legacy sentence multilingual space mismatch fixed by product C (aligned pack). **In-memory embed matrix** + rayon; optional background HNSW; SQL BLOB fallback; invalidate on write/clear/library switch. Face cosine dim fail-closed. Dedup Similar dHash (schema v9). Plan: `docs/superpowers/plans/2026-07-24-ai-search-stopbleed-and-embed-cache.md` + ANN/pHash design. Patterns: `change-image-search-model.md`, `change-library-perf.md`.
