...
- **Image-search model tracks (2026-07-23):** **A stop-bleed shipped**. **B0 abandoned.** Default = **CLIP B/32**. **Track B Phase 0:** SigLIP2 pack — Python ORT OK; Rust ort **FAIL int8 / PASS quantized+fp16**; prefer **`vision_model_quantized` + `text_model_quantized`**. Scripts `probe_siglip2_onnx.py` + `probe_siglip2_ort/`; guide `docs/guide/siglip2-phase0-probe.md`. Next: real-album smoke before product UI.
