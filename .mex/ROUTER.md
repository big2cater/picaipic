...
- **S1 small-step + CRUD fixture (2026-07-27):** `AFile::new` delegates header/EXIF/orientation/identity/descriptions/capture extraction and RAW metadata merge policy to focused helpers; CRUD retains connection-backed cores for temporary SQLite coverage. Fixtures cover TIFF fields plus pure `RawMeta` overlay behavior. `t_sqlite` suite: 32 passed — `patterns/test-sqlite-crud-fixture.md`.
