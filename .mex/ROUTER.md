...
- **S1 small-step + CRUD fixture (2026-07-27):** `AFile::new` delegates 128 KiB image/RAW header pre-read, EXIF read/fallback policy, and orientation fallback to focused helpers; CRUD `insert/update/delete` retain production wrappers with `*_with_conn` cores for temporary SQLite integration coverage. `t_sqlite` suite: 27 passed — `patterns/test-sqlite-crud-fixture.md`.
