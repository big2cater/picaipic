...
- **S1/S6 SQLite audit follow-up (2026-07-27):** `AFile::new` delegates header/EXIF/orientation/identity/descriptions/capture extraction and RAW metadata merge policy to focused helpers; CRUD retains connection-backed cores for temporary SQLite coverage. Fixtures cover TIFF fields plus pure `RawMeta` overlay behavior. Stop at this orchestration boundary: Live/Motion/HEIC, geocode, AI prompt and final assembly stay in `new`. Validation: `cargo check`; full `cargo test` **107 passed / 1 ignored** — `patterns/test-sqlite-crud-fixture.md`, `docs/review/code-review-2026-07-26-supplement.md`.
- **Scan profiling (2026-07-27):** local `D:\100k` all-JPEG reference measured via ignored `AFile::new` fixture: 10k and 100k both **43.7 files/sec**, 0 failures. Scope is metadata/header/GPS pre-read only; full scan still needs DB/thumb/embed phase profiling. Protocol and decision rules: `docs/guide/library-scan-profiling.md`, `patterns/profile-library-scan.md`.
