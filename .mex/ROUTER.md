...
- **S1 small-step + CRUD fixture (2026-07-27):** `AFile::new` delegates header pre-read, EXIF read/fallback, orientation, and core camera/date identity extraction to focused helpers; CRUD retains connection-backed cores for temporary SQLite integration coverage. Minimal TIFF fixtures cover IFD0 + ExifIFD. `t_sqlite` suite: 28 passed — `patterns/test-sqlite-crud-fixture.md`.
