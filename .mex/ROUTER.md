...
- **S1 small-step + CRUD fixture (2026-07-27):** `AFile::new` delegates header pre-read, EXIF read/fallback, orientation, camera/date identity, and descriptive EXIF extraction to focused helpers; CRUD retains connection-backed cores for temporary SQLite integration coverage. Minimal TIFF fixtures cover IFD0 + ExifIFD + UserComment. `t_sqlite` suite: 29 passed — `patterns/test-sqlite-crud-fixture.md`.
