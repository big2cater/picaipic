...
- **S1 small-step + CRUD fixture (2026-07-27):** `AFile::new` delegates header pre-read, EXIF read/fallback, orientation, identity, descriptions, and lens/capture extraction to focused helpers; CRUD retains connection-backed cores for temporary SQLite integration coverage. Minimal TIFF fixtures cover IFD0, ExifIFD, UserComment, lens tags, Rational/SRational exposure fields, and inline ISO. `t_sqlite` suite: 30 passed — `patterns/test-sqlite-crud-fixture.md`.
