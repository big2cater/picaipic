...
| **Photo effect** | `PhotoVortexLayer.vue` + `GridView.vue` | Freeze thumbs → UV spiral; **theme-gated mount**; ResizeObserver size cache (no per-frame reflow); lazy GL init; pending capture rAF cancelled on stop/unmount |
