...
## Context
PicAiPic keeps a database per library. `t_migration.rs` owns forward schema evolution, `t_sqlite.rs` owns queries/models, and `t_storage.rs` owns paths, custom storage, WAL checkpointing, backup, and restore.
