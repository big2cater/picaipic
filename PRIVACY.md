...
PicAiPic is designed as a local-first photo manager. Your photo library is processed on your device, and your files remain under your control.

This document explains what data PicAiPic accesses, what data PicAiPic does not collect by default, and when network access may occur.

## What PicAiPic Accesses

PicAiPic may access the following data on your device in order to provide its features:
...
PicAiPic uses this data to support browsing, search, deduplication, tagging, ratings, file type filtering, video support, face clustering, and other library management features.

## Local Processing

PicAiPic is intended to process your library locally on your device.

By default:

- Your photos and videos are not uploaded to a PicAiPic cloud service
...
- PicAiPic does not include advertising trackers
- PicAiPic does not track daily active usage, session counts, or feature usage patterns
- The frontend does not record user behavior, clicks, or navigation events

## Network Access

PicAiPic may access the network in limited cases where the feature requires it. Based on the current implementation, this may include:
...
PicAiPic includes Aptabase, a privacy-first analytics service, in its backend. It is enabled by default in release builds to help the maintainer understand basic adoption and stability.
...
When enabled, PicAiPic sends exactly two app-lifecycle events:

- App started
- App exited

Each event carries only the PicAiPic version, device platform, and operating system. No coarse region or IP-derived location is stored or surfaced by the analytics service.
...
**PicAiPic does not and will never send** your photos, videos, folder paths, filenames, search queries, tags, ratings, comments, EXIF data, embeddings, face clusters, thumbnails, previews, database contents, or any other library data.

## Data Storage

PicAiPic stores application data locally on your device. This may include:
...
PicAiPic does not provide its own cloud storage service.
...
[https://github.com/big2cater/picaipic](https://github.com/big2cater/picaipic)
