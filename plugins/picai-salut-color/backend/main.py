...
from salut_adapter import SalutAdapter


PLUGIN_ID = "picai-salut-color"
PORT = int(os.environ.get("PICAIPIC_PLUGIN_PORT", "8011"))
...
class Handler(BaseHTTPRequestHandler):
    server_version = "PicAiPicSalut/0.1"
    ...
    def do_POST(self) -> None:
        ...
        if path == "/invoke/color-transfer":
            status, response = ADAPTER.color_transfer(payload)
            json_response(self, status, response)
            return
        ...
        if path == "/invoke/export-lut":
            ...
            json_response(self, 501, {
                ...
                "error": {
                    ...
                    "message": "SA-LUT LUT export is declared for the host contract but is not implemented yet.",
                },
            })
            return
...
def main() -> None:
    ...
    print(f"salut backend: {ADAPTER.source_root}", flush=True)
