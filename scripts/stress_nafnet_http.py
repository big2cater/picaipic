...
PLUGIN_ROOT = ROOT / "plugins" / "picai-nafnet-restore"
...
def wait_health(base_url: str, timeout_s: float) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if status == 200 and payload.get("pluginId") == "picai-nafnet-restore":
                return
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stress PicAiPic NAFNet async HTTP protocol with mock work.")
    ...
    env["NAFNET_SOURCE_DIR"] = os.environ.get("NAFNET_SOURCE_DIR", r"D:\ailab\NAFNet")
    ...
    try:
        ...
        for index in range(args.tasks):
            task_id = f"nafnet-http-stress-{int(time.time() * 1000)}-{index}"
            status, payload = request_json("POST", f"{base_url}/invoke/denoise", {
                "taskId": task_id,
                "inputs": {},
                "parameters": {
                    "mockTask": True,
        ...
        running_id = f"nafnet-http-running-cancel-{int(time.time() * 1000)}"
        request_json("POST", f"{base_url}/invoke/denoise", {
            "taskId": running_id,
            "inputs": {},
            "parameters": {
                "mockTask": True,
