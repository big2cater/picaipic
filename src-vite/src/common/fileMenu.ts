...
export const useFileMenuItems = (
  ...
  onAction: (action: string | { type: string; pluginId: string; capabilityId: string; menuId: string }) => void,
  pluginMenus?: Ref<PluginMenuItem[]>
) => {
  return computed(() => {
    const f = file.value;
    if (!f) return [];
    ...
    const pluginMenuItems = (pluginMenus?.value || [])
      .map((menu) => ({
        label: menu.label,
        icon: markRaw(pluginIconMap[menu.icon || ''] || IconPalette),
        disabled: !isImage,
        action: () => onAction({
          type: 'plugin-menu',
