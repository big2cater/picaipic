<template>
  <section
    ...
    class="collection-tray min-h-0 flex flex-col border-t border-base-content/5 shadow-sm bg-base-200/40"
    :class="libConfig.activePane === 'collection' ? '' : 'opacity-95'"
  >
    <div class="sidebar-panel-header cursor-pointer select-none" @click="$emit('toggle-expanded')">
      <TButton
        :icon="IconRight"
        :buttonSize="'small'"
    ...
    <div v-if="expanded" class="min-h-0 flex-1 overflow-y-auto pb-1">
      <div
        ...
        :class="[
          ...
          isItemDragging ? 'border-dashed border-primary/40' : '',
        ]"
        @click="selectCollection(collection)"
      >
        <IconTag class="mx-1 w-5 h-5 shrink-0" />
        <input
          v-if="renamingId === collection.id"
          ref="renameInputRef"
          v-model="renameValue"
          class="input input-xs px-1 min-w-0 flex-1 text-sm"
          maxlength="64"
          @click.stop
          @mousedown.stop
        ...
        <div
          v-if="renamingId !== collection.id"
          :class="[selectedId === collection.id ? '' : 'hidden group-hover:block']"
        >
          <ContextMenu
            :iconMenu="IconMore"
...
import { IconAdd, IconRight, IconEdit, IconMore, IconTag, IconTrash } from '@/common/icons';
...
async function loadCollections(preferredId?: number) {
  ...
  const nextSelectedId = Number(
    preferredId || libConfig.collection?.selectedId || selectedId.value || 0,
  );
  ...
  if (selected) {
    ...
    if (libConfig.collection) libConfig.collection.selectedId = null;
  }
}
...
async function addCollection() {
  ...
  const collection = await createCollection(
    t('collection.default_name', { index: collections.value.length + 1 }),
  );
