...
const pluginActionDialog = ref({
  ...
  taskProgress: 0 as number,
  taskMessage: '' as string,
});
...
async function runPluginAction(form: { inputs: Record<string, any>; parameters: Record<string, any> }) {
  ...
  try {
    ...
    if (destination) {
      ...
      const imported = await importFile(outputPath, destination.folderId, destination.folderPath);
      if (imported) {
...
function handleItemAction(payload: { action: any, index: number }) {
  ...
  if (action?.type === 'plugin-menu') {
    void runPluginMenuAction(action, fileList.value[selectedItemIndex.value]);
    return;
  }

  if (typeof action === 'string' && action.startsWith('rating-')) {
